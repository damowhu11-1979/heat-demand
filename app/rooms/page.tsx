...
/* ------------ base-path helpers (static export / GH Pages safe) ------------ */
...
function toPath(p: string): string {
  const base = repoBase();
  const cleaned = p.replace(/^\/+/, ''); // remove any leading slash
  return `${base}${cleaned}`;
}
  ...
  return (
    <main style={wrap}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
        {/* Back to ventilation (static-safe) */}
        <a href={toPath('ventilation/')} style={{ ...secondaryBtn, textDecoration: 'none' as const }}>
          ← Back: Ventilation
        </a>
        {/* Next to Building Elements (static-safe) */}
        <a href={toPath('elements/')} style={{ ...primaryBtn, textDecoration: 'none' as const }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, intermittentPct: e.target.value === '' ? undefined : Number(e.target.value) })
                  }
                  style={{ maxWidth: 160 }}
                />
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, heatGainsW: e.target.value === '' ? undefined : Number(e.target.value) })
                  }
                  style={{ maxWidth: 160 }}
                />
...
const primaryBtn: React.CSSProperties = {
  background: '#111', color: '#fff', border: '1px solid #111', padding: '10px 16px', borderRadius: 12, cursor: 'pointer',
};
const secondaryBtn: React.CSSProperties = {
  background: '#fff', color: '#111', border: '1px solid #ddd', padding: '10px 16px', borderRadius: 12, cursor: 'pointer',
};
