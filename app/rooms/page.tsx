...
const readRooms = (): Zone[] | null => {
  ...
  try { const raw = localStorage.getItem(ROOMS_KEY); return raw ? JSON.parse(raw) : null; } catch { return null; }
};
const writeRooms = (zones: Zone[]) => {
  if (typeof window === 'undefined') return;
  try { localStorage.setItem(ROOMS_KEY, JSON.stringify(zones)); } catch {}
};
...
export default function RoomsPage(): React.JSX.Element {
  ...
  // auto-save zones whenever they change (debounced)
  useEffect(() => {
  ...
  return (
    <main style={wrap}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
        <Link href="/ventilation" style={{ ...secondaryBtn, textDecoration: 'none' }}>← Back: Ventilation</Link>
          ...
          <div style={modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <Input type="number" step="0.01" value={form.maxCeiling} onChange={(e) => setForm({ ...form, maxCeiling: Number(e.target.value || 0) })} style={{ maxWidth: 160 }} />
            ...
            <div>
              ...
              <p style={hint}>Adds a percentage allowance for heat-up on top of fabric loss.</p>
            </div>
            ...
            <div>
              ...
              <p style={hint}>Add extra gains (e.g., occupancy, machinery, solar).</p>
            </div>
...
const rowHeader: React.CSSProperties = { display: 'flex', gap: 8, padding: '8px 4px', color: '#555', fontSize: 12, borderBottom: '1px solid #eee' };
const row: React.CSSProperties = { display: 'flex', gap: 8, padding: '10px 4px', alignItems: 'center', borderBottom: '1px solid #f2f2f2' };
...
const modal: React.CSSProperties = { width: 'min(720px, 92vw)', background: '#fff', borderRadius: 16, border: '1px solid #e6e6e6', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', padding: 18 };
